...
func isolationFromPayload(payload map[string]interface{}) isolationSpec {
	if getBoolPayload(payload, "isolationRelaxed") {
	...
	if getBoolPayload(payload, "allowPrivilegeEscalation") {
		spec.NoNewPrivileges = false
	}
	if getBoolPayload(payload, "privileged") {
	...
	if getBoolPayload(payload, "readOnlyRoot") {
		spec.ReadOnly = true
